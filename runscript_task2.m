...
for i = 1:length(vels_u3)
    vels_pure(i) = split_vel_finder_task2(vels_u3(i), g, roughness, rho);
    vels_u4(i) = double(vels_pure(i).u_4(1,1));
    vels_u5(i) = double(vels_pure(i).u_5(1,1));
    cf_4(i) = double(vels_pure(i).cf_4(1,1));
    cf_5(i) = double(vels_pure(i).cf_5(1,1));
    p1_vals(i) = (1.3 * rho * g) - (vels_u3(i).^2 * rho);
    p3_vals(i) = rho * (vels_u4(i).^2 - (0.5 * vels_u3(i).^2) + (2 * cf_4(i) * vels_u4(i).^2 * l_c / d_c));
    p2_vals(i) = p3_vals(i) + (vels_u3(i).^2 * rho * 0.5) * (1 + (4 * cf_finder(vels_u3(i), roughness, d_a) * l_a / d_a)) + (6 * rho * g);

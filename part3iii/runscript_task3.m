...
for i = 1:length(vels_u3)
    ...
    vels_pure(i) = split_vel_finder_task3(vels_u3(i), g, roughness, rho);
    ...
    p3_vals(i) = rho * (11 * vels_u4(i).^2 - (0.5 * vels_u3(i).^2) + (2 * cf_4(i) * vels_u4(i).^2 * l_c / d_c));
    p2_vals(i) = p3_vals(i) + (vels_u3(i).^2 * rho * 0.5) * (121 + (4 * cf_3(i) * l_a / d_a)) + (6 * rho * g);
